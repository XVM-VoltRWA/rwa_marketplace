...
import { Client, Wallet } from "npm:xrpl@4.4.1";
import NftService from "../_shared/nft/service.ts";
import config, { getExplorerBase, getNetworkUrl } from "../_shared/config/index.ts";
import type { CreateNftRequest, CreateNftResponse } from "./type.ts";
...
Deno.serve(async (req) => {
  ...
  const { name, image_url, metadata, owner_address } = body;
  if (!name || !image_url) {
    return new Response(
  ...
  if (!config.XUMM_API_KEY)
    throw new Error("XUMM API key/secret not configured; skipping XUMM payload creation");
  ...
  try {
    ...
    if (owner_address) {
      const giveNftToCreatorResult = await nftService.giveNftToCreator(mintResult.nftTokenId, owner_address, { amount: "0", flags: 1 }, backendWallet);
      ...
      if (giveNftToCreatorResult && giveNftToCreatorResult.offerIndex && config.XUMM_API_KEY && config.XUMM_API_SECRET) {
        try {
